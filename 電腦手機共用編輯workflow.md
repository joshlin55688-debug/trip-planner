# 電腦手機共用編輯 Workflow：把一個電腦上的專案接上「手機可編輯 + 自動部署」

這份記錄的是把 `trip-planner` 從「只在電腦本機開發」升級成「手機 Claude 可以直接改、改完自動上線」的完整步驟。以後任何專案要做一樣的事（例如你說「我要讓這個專案在手機上也可以編輯」），照這份走一遍即可，不用重新摸索。
